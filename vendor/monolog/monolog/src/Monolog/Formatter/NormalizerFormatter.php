...
class NormalizerFormatter implements \Woosa\Adyen\Monolog\Formatter\FormatterInterface
{
    ...
    protected function normalizeException(\Throwable $e, int $depth = 0)
    {
        ...
        $data = ['class' => \Woosa\Adyen\Monolog\Utils::getClass($e), 'message' => $e->getMessage(), 'code' => (int) $e->getCode(), 'file' => $e->getFile() . ':' . $e->getLine()];
    ...
    {
        return \Woosa\Adyen\Monolog\Utils::jsonEncode($data, $this->jsonEncodeOptions, $ignoreErrors);
    }
    protected function formatDate(\DateTimeInterface $date)
    {
        // in case the date format isn't custom then we defer to the custom DateTimeImmutable
        // formatting logic, which will pick the right format based on whether useMicroseconds is on
        if ($this->dateFormat === self::SIMPLE_DATE && $date instanceof \Woosa\Adyen\Monolog\DateTimeImmutable) {
            return (string) $date;
        }
