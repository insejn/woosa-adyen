...
use Woosa\Adyen\PhpConsole\Connector;
use Woosa\Adyen\PhpConsole\Handler as VendorPhpConsoleHandler;
use Woosa\Adyen\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \Woosa\Adyen\Monolog\Handler\AbstractProcessingHandler
{
    private $options = [
        'enabled' => \true,
     ...
     */
    public function __construct(array $options = [], ?\Woosa\Adyen\PhpConsole\Connector $connector = null, $level = \Woosa\Adyen\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if (!\class_exists('Woosa\\Adyen\\PhpConsole\\Connector')) {
    ...
    private function initConnector(?\Woosa\Adyen\PhpConsole\Connector $connector = null) : \Woosa\Adyen\PhpConsole\Connector
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \Woosa\Adyen\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \Woosa\Adyen\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\Woosa\Adyen\PhpConsole\Helper::isRegistered()) {
            \Woosa\Adyen\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \Woosa\Adyen\PhpConsole\Handler::getInstance();
    ...
    public function getConnector() : \Woosa\Adyen\PhpConsole\Connector
    {
    ...
    {
        if ($record['level'] < \Woosa\Adyen\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record) : void
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \Woosa\Adyen\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \Woosa\Adyen\Monolog\Formatter\LineFormatter('%message%');
    }
}
