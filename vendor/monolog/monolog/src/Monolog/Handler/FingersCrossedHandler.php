...
use Woosa\Adyen\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy;
use Woosa\Adyen\Monolog\Handler\FingersCrossed\ActivationStrategyInterface;
...
class FingersCrossedHandler extends \Woosa\Adyen\Monolog\Handler\Handler implements \Woosa\Adyen\Monolog\Handler\ProcessableHandlerInterface, \Woosa\Adyen\Monolog\ResettableInterface, \Woosa\Adyen\Monolog\Handler\FormattableHandlerInterface
{
    ...
    public function __construct($handler, $activationStrategy = null, int $bufferSize = 0, bool $bubble = \true, bool $stopBuffering = \true, $passthruLevel = null)
    {
        if (null === $activationStrategy) {
            $activationStrategy = new \Woosa\Adyen\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy(\Woosa\Adyen\Monolog\Logger::WARNING);
        }
        // convert simple int activationStrategy to an object
        if (!$activationStrategy instanceof \Woosa\Adyen\Monolog\Handler\FingersCrossed\ActivationStrategyInterface) {
            $activationStrategy = new \Woosa\Adyen\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy($activationStrategy);
        }
        ...
        if ($passthruLevel !== null) {
            $this->passthruLevel = \Woosa\Adyen\Monolog\Logger::toMonologLevel($passthruLevel);
        }
    ...
    public function reset()
    {
        ...
        if ($this->getHandler() instanceof \Woosa\Adyen\Monolog\ResettableInterface) {
