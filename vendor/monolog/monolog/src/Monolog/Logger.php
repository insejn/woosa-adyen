 ...
 */
namespace Woosa\Adyen\Monolog;

use DateTimeZone;
use Woosa\Adyen\Monolog\Handler\HandlerInterface;
use Woosa\Adyen\Psr\Log\LoggerInterface;
use Woosa\Adyen\Psr\Log\InvalidArgumentException;
use Throwable;
/**
 * Monolog log channel
 *
 ...
 */
class Logger implements \Woosa\Adyen\Psr\Log\LoggerInterface, \Woosa\Adyen\Monolog\ResettableInterface
{
    /**
     ...
     */
    public function pushHandler(\Woosa\Adyen\Monolog\Handler\HandlerInterface $handler) : self
    {
     ...
     */
    public function popHandler() : \Woosa\Adyen\Monolog\Handler\HandlerInterface
    {
        if (!$this->handlers) {
    ...
    {
        foreach ($this->handlers as $handler) {
            if ($handler instanceof \Woosa\Adyen\Monolog\ResettableInterface) {
                $handler->reset();
            }
        }
        foreach ($this->processors as $processor) {
            if ($processor instanceof \Woosa\Adyen\Monolog\ResettableInterface) {
                $processor->reset();
            }
        }
    }
    ...
    public static function getLevelName(int $level) : string
    {
        if (!isset(static::$levels[$level])) {
            throw new \Woosa\Adyen\Psr\Log\InvalidArgumentException('Level "' . $level . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels)));
        }
    ...
    public static function toMonologLevel($level) : int
    {
        ...
        if (!\is_int($level)) {
            throw new \Woosa\Adyen\Psr\Log\InvalidArgumentException('Level "' . \var_export($level, \true) . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels)));
        }
        return $level;
    }
    /**
