...
class ErrorHandler
{
    ...
    public function __construct(\Woosa\Adyen\Psr\Log\LoggerInterface $logger)
    {
        $this->logger = $logger;
    }
    /**
    ...
    {
        return ['ParseError' => \Woosa\Adyen\Psr\Log\LogLevel::CRITICAL, 'Throwable' => \Woosa\Adyen\Psr\Log\LogLevel::ERROR];
    }
    ...
    private function handleException(\Throwable $e)
    {
        ...
        $this->logger->log($level, \sprintf('Uncaught Exception %s: "%s" at %s line %s', \Woosa\Adyen\Monolog\Utils::getClass($e), $e->getMessage(), $e->getFile(), $e->getLine()), ['exception' => $e]);
    ...
    public function handleError($code, $message, $file = '', $line = 0, $context = [])
    {
        ...
        if (!$this->hasFatalErrorHandler || !\in_array($code, self::$fatalErrors, \true)) {
            $level = $this->errorLevelMap[$code] ?? \Woosa\Adyen\Psr\Log\LogLevel::CRITICAL;
    ...
    public function handleFatalError()
    {
        ...
        if ($lastError && \in_array($lastError['type'], self::$fatalErrors, \true)) {
            $this->logger->log($this->fatalLevel === null ? \Woosa\Adyen\Psr\Log\LogLevel::ALERT : $this->fatalLevel, 'Fatal Error (' . self::codeToString($lastError['type']) . '): ' . $lastError['message'], ['code' => $lastError['type'], 'message' => $lastError['message'], 'file' => $lastError['file'], 'line' => $lastError['line'], 'trace' => $this->lastFatalTrace]);
            if ($this->logger instanceof \Woosa\Adyen\Monolog\Logger) {
