...
class CurlClient implements \Woosa\Adyen\Adyen\HttpClient\ClientInterface
{
     ...
     */
    public function requestJson(\Woosa\Adyen\Adyen\Service $service, $requestUrl, $params, $requestOptions = null)
    {
        ...
        $userAgent = $config->get('applicationName') . " " . \Woosa\Adyen\Adyen\Client::USER_AGENT_SUFFIX . $client->getLibraryVersion();
    ...
    public function curlSetHttpProxy($ch, $httpProxy)
    {
        ...
        if ($urlParts == \false || !\array_key_exists("host", $urlParts)) {
            throw new \Woosa\Adyen\Adyen\AdyenException("Invalid proxy configuration " . $httpProxy);
        }
        $proxy = $urlParts["host"];
    ...
    public function requestPost(\Woosa\Adyen\Adyen\Service $service, $requestUrl, $params)
    {
        ...
        $userAgent = $config->get('applicationName') . " " . \Woosa\Adyen\Adyen\Client::USER_AGENT_SUFFIX . $client->getLibraryVersion();
        //Set the content type to application/json and use the defined userAgent
        $headers = array('Content-Type: application/x-www-form-urlencoded', 'User-Agent: ' . $userAgent);
        \curl_setopt($ch, \CURLOPT_HTTPHEADER, $headers);
        // return the result
        \curl_setopt($ch, \CURLOPT_RETURNTRANSFER, 1);
        //Execute the request
    ...
    protected function handleCurlError($url, $errno, $message, $logger)
    {
        switch ($errno) {
            ...
            case \CURLE_OPERATION_TIMEOUTED:
                $msg = "Could not connect to Adyen ({$url}).  Please check your " . "internet connection and try again.";
                break;
            case \CURLE_SSL_CACERT:
            case \CURLE_SSL_PEER_CERTIFICATE:
                $msg = "Could not verify Adyen's SSL certificate.  Please make sure " . "that your network is not intercepting certificates.  " . "(Try going to {$url} in your browser.)  " . "If this problem persists,";
                break;
            default:
        ...
        $logger->error($msg);
        throw new \Woosa\Adyen\Adyen\ConnectionException($msg, $errno);
    }
    /**
