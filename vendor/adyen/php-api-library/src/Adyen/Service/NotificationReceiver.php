...
use Woosa\Adyen\Adyen\Exception\AuthenticationException;
use Woosa\Adyen\Adyen\Exception\HMACKeyValidationException;
use Woosa\Adyen\Adyen\Exception\MerchantAccountCodeException;
use Woosa\Adyen\Adyen\Util\HmacSignature;
class NotificationReceiver
{
     ...
     */
    public function __construct(\Woosa\Adyen\Adyen\Util\HmacSignature $hmacSignature)
    {
    ...
    public function validateHmac($response, $hmacKey)
    {
        ...
        if (!$this->hmacSignature->isValidNotificationHMAC($hmacKey, $response)) {
            if ($isTestNotification) {
                ...
                throw new \Woosa\Adyen\Adyen\Exception\HMACKeyValidationException($message);
            }
            return \false;
        }
        return \true;
    ...
    public function isAuthenticated($response, $merchantAccount, $notificationUsername, $notificationPassword)
    {
        ...
        if (empty($submittedMerchantAccount) || empty($merchantAccount)) {
            if ($isTestNotification) {
                throw new \Woosa\Adyen\Adyen\Exception\MerchantAccountCodeException('merchantAccountCode is empty in settings or in the notification');
            }
            return \false;
        }
        ...
        if (!isset($_SERVER['PHP_AUTH_USER']) || !isset($_SERVER['PHP_AUTH_PW'])) {
            if ($isTestNotification) {
                ...
                throw new \Woosa\Adyen\Adyen\Exception\AuthenticationException($message);
            }
            return \false;
        }
    ...
    public function validateNotificationMode($notificationMode, $testMode)
    {
        ...
        if ($testMode && ($notificationMode === 'false' || $notificationMode === \false) || !$testMode && ($notificationMode === 'true' || $notificationMode === \true)) {
            return \true;
        }
        return \false;
    }
    ...
    {
        if (\strpos(\strtolower($pspReference), 'test_') !== \false || \strpos(\strtolower($pspReference), 'testnotification_') !== \false) {
            return \true;
        }
        return \false;
    }
