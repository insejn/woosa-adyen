...
class Client
{
    ...
    {
        if ($environment == \Woosa\Adyen\Adyen\Environment::TEST) {
            $this->config->set('environment', \Woosa\Adyen\Adyen\Environment::TEST);
            ...
            if ($liveEndpointUrlPrefix) {
                ...
                $this->config->set('endpointCheckout', null);
                // not supported please specify unique identifier
            }
        } else {
            // environment does not exist
            $msg = "This environment does not exist, use " . \Woosa\Adyen\Adyen\Environment::TEST . ' or ' . \Woosa\Adyen\Adyen\Environment::LIVE;
    ...
    {
        $this->config->set('externalPlatform', array('name' => $name, 'version' => $version, 'integrator' => $integrator));
    }
    /**
    ...
    {
        return new \Woosa\Adyen\Adyen\HttpClient\CurlClient();
    }
    /**
    ...
    {
        $logger = new \Woosa\Adyen\Monolog\Logger('adyen-php-api-library');
        $logger->pushHandler(new \Woosa\Adyen\Monolog\Handler\StreamHandler('php://stderr', \Woosa\Adyen\Monolog\Logger::NOTICE));
        return $logger;
    }
}
