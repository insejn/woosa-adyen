...
namespace Woosa\Adyen\VIISON\AddressSplitter;

use Woosa\Adyen\VIISON\AddressSplitter\Exceptions\SplittingException;
/**
...
class AddressSplitter
{
    ...
    public static function splitAddress($address)
    {
        ...
        if ($result === 0) {
            throw new \Woosa\Adyen\VIISON\AddressSplitter\Exceptions\SplittingException(\Woosa\Adyen\VIISON\AddressSplitter\Exceptions\SplittingException::CODE_ADDRESS_SPLITTING_ERROR, $address);
        } elseif ($result === \false) {
            throw new \RuntimeException(\sprintf('Error occurred while trying to split address \'%s\'', $address));
        }
        if (!empty($matches['A_Street_name'])) {
            return array('additionToAddress1' => $matches['A_Addition_to_address_1'], 'streetName' => $matches['A_Street_name'], 'houseNumber' => $matches['A_House_number_match'], 'houseNumberParts' => array('base' => $matches['A_House_number_base'], 'extension' => isset($matches['A_House_number_extension']) ? \trim($matches['A_House_number_extension']) : ''), 'additionToAddress2' => isset($matches['A_Addition_to_address_2']) ? $matches['A_Addition_to_address_2'] : '');
        } else {
            return array('additionToAddress1' => $matches['B_Addition_to_address_1'], 'streetName' => $matches['B_Street_name'], 'houseNumber' => $matches['B_House_number_match'], 'houseNumberParts' => array('base' => $matches['B_House_number_base'], 'extension' => isset($matches['B_House_number_extension']) ? \trim($matches['B_House_number_extension']) : ''), 'additionToAddress2' => isset($matches['B_Addition_to_address_2']) ? $matches['B_Addition_to_address_2'] : '');
        }
    }
    /**
    ...
    public static function splitHouseNumber($houseNumber)
    {
        $regex = '/
            ...
            /xu';
        // Option (u)nicode and e(x)tended syntax
        $result = \preg_match($regex, $houseNumber, $matches);
        if ($result === 0) {
            throw new \Woosa\Adyen\VIISON\AddressSplitter\Exceptions\SplittingException(\Woosa\Adyen\VIISON\AddressSplitter\Exceptions\SplittingException::CODE_HOUSE_NUMBER_SPLITTING_ERROR, $houseNumber);
        } elseif ($result === \false) {
            throw new \RuntimeException(\sprintf('Error occurred while trying to house number \'%s\'', $houseNumber));
        }
        return array('base' => $matches['House_number_base'], 'extension' => $matches['House_number_extension']);
    }
}
